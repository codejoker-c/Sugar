...
def _initialize_radiuses_gauss_rasterizer(sugar):
    ...
    # initialize scale as the second min distance to the current gaussian
    # initialize quaternion as the eye matrix
    # why?
...
class SuGaR(nn.Module):
    ...
    def triangle_border_edges(self):
        ...
        edges = edges + 4 * torch.arange(len(self.points), device=self.device)[:, None,
                            None]  # Shape: (n_pts, n_border_edges_per_gaussian, 2)
        edges = edges.view(-1, 2)  # Shape: (n_pts * n_border_edges_per_gaussian, 2)
        return edges

    @property
    def triangles(self):
        triangles = self.primitive_triangles[None].expand(self.n_points, -1,
                                                          -1).clone()  # Shape: (n_pts, n_triangles_per_gaussian, 3)
        triangles = triangles + 4 * torch.arange(len(self.points), device=self.device)[:, None,
                                    None]  # Shape: (n_pts, n_triangles_per_gaussian, 3)
    ...
    def unbind_surface_mesh(self):
        self._quaternions = nn.Parameter(self.quaternions.detach(), requires_grad=self.learn_quaternions).to(
            self.nerfmodel.device)
        self._scales = nn.Parameter(scale_inverse_activation(self.scaling.detach()),
                                    requires_grad=self.learn_scales).to(self.nerfmodel.device)
        self.scale_activation = scale_activation
        self.scale_inverse_activation = scale_inverse_activation
        self.binded_to_surface_mesh = False
    ...
    def splat_mesh(self, p3d_camera,
        ...
        new_verts_in_camera_space = p3d_camera.get_world_to_view_transform().transform_points(
            new_verts.view(-1, 3)).reshape(n_gaussians, -1, 3)  # Shape (n_points, n_vertices_per_gaussian, 3)
        gaussian_centers_in_camera_space = p3d_camera.get_world_to_view_transform().transform_points(
            gaussian_centers.view(-1, 3)).reshape(n_gaussians, -1, 3)  # Shape (n_points, 1, 3)

        if mode == 'depth':
            new_verts_in_camera_space[..., 2] = gaussian_centers_in_camera_space[..., 2]
        else:
            # stretch the vector by (distance between gaussian and camera center / points projection along gaussians vector) in camera space
            ...
            centers_projection = (gaussian_centers_in_camera_space * proj_dir).sum(-1,
                                                                                   keepdim=True)  # Shape (n_points, 1, 1)
        ...
        splatted_verts = p3d_camera.get_world_to_view_transform().inverse().transform_points(
            new_verts_in_camera_space.reshape(-1, 3))
    ...
    def prune_points(self, prune_mask):
        ...
        self._sh_coordinates_rest = torch.nn.Parameter(self._sh_coordinates_rest[prune_mask].detach(),
                                                       requires_grad=False)
    ...
    def compute_gaussian_overlap_with_neighbors(
        ...
        neighbor_center_to_samples = gaussian_samples.unsqueeze(1) - self.points[
            neighbor_idx[:, neighbor_start_idx:]].unsqueeze(
            2)  # (n_points, n_neighbors-1, n_samples_to_compute_overlap, 3)
        ...
        inverse_scales = 1. / scaling[neighbor_idx[:, neighbor_start_idx:]].unsqueeze(
            2)  # (n_points, n_neighbors-1, 1, 3)
        ...
        gaussian_weights = torch.exp(
            -1. / 2. * (gaussian_distances ** 2).sum(dim=-1))  # (n_points, n_neighbors-1, n_samples_to_compute_overlap)
    ...
    def compute_level_surface_points_from_camera(
        ...
        gaussian_standard_deviations = (
                self.scaling * quaternion_apply(quaternion_invert(self.quaternions), gaussian_to_camera)).norm(
            dim=-1)
        points_stds = gaussian_standard_deviations[closest_gaussians_idx[..., 0]]

        # We compute ray samples
        points_range = torch.linspace(-range_size, range_size, n_points_in_range).to(self.device).view(1, -1,
                                                                                                       1)  # (1, n_points_in_range, 1)
        points_range = points_range * points_stds[..., None, None].expand(-1, n_points_in_range,
                                                                          1)  # (n_points, n_points_in_range, 1)
        camera_to_samples = torch.nn.functional.normalize(all_world_points - fov_cameras.get_camera_center(),
                                                          dim=-1)  # (n_points, 3)
        samples = (all_world_points[:, None, :] + points_range * camera_to_samples[:, None, :]).view(-1,
                                                                                                     3)  # (n_points * n_points_in_range, 3)
        samples_closest_gaussians_idx = closest_gaussians_idx[:, None, :].expand(-1, n_points_in_range, -1).reshape(-1,
                                                                                                                    self.knn_to_track)
        ...
        for i in range(0, len(samples), n_points_per_pass):
            ...
            if not compute_intersection_for_flat_gaussian:
                ...
                neighbor_opacities = (shift * closest_gaussian_normals).sum(dim=-1).pow(2) / (
        ...
        if not just_use_depth_as_level:
            ...
            intersection_t = (surface_level - value_before_level) / (first_value_above_level - value_before_level) * (
                    first_t_above_level - t_before_level) + t_before_level
            intersection_points = (
                    all_world_points[~empty_pixels] + intersection_t[:, None] * camera_to_samples[~empty_pixels])
        else:
        ...
        if return_depth or return_gaussian_idx or return_normals:
            ...
            if return_normals:
                ...
                if not compute_intersection_for_flat_gaussian:
                    warped_shift = closest_gaussian_inv_scaled_rotation.transpose(-1, -2) @ shift[..., None]
                    neighbor_opacities = (warped_shift[..., 0] * warped_shift[..., 0]).sum(dim=-1).clamp(min=0.,
                                                                                                         max=1e8)
                else:
                ...
                if not compute_flat_normals:
                    density_grad = (
                            neighbor_opacities[..., None] * (closest_gaussian_inv_scaled_rotation @ warped_shift)[
                        ..., 0]).sum(dim=-2)
                else:
                    closest_gaussian_normals = self.get_normals()[points_closest_gaussians_idx]
                    closest_gaussian_min_scales = self.scaling.min(dim=-1, keepdim=True)[0][
                        points_closest_gaussians_idx]
                    density_grad = (
                            neighbor_opacities[..., None] *
                            1. / (closest_gaussian_min_scales).pow(2) * (shift * closest_gaussian_normals).sum(dim=-1,
                                                                                                               keepdim=True) * closest_gaussian_normals
                    ).sum(dim=-2)
            ...
            if not just_use_depth_as_level:
                ...
                intersection_t = (surface_level - value_before_level) / (
                        first_value_above_level - value_before_level) * (
                                         first_t_above_level - t_before_level) + t_before_level
                intersection_points = (all_world_points[~empty_pixels] + intersection_t[:, None] * camera_to_samples[
                    ~empty_pixels])
            else:
    ...
    def render_image_gaussian_rasterizer(
        ...
        if point_colors is None:
            if not compute_color_in_rasterizer:
                if sh_rotations is None:
                    ...
                    splat_colors = self.sh_coordinates.view(-1, 3)
                else:
                    splat_colors = self.get_points_rgb(
                        positions=positions,
                        camera_centers=None,
        ...
        rendered_image, radii = rasterizer(
            ...
            # scales=self.nerfmodel.gaussians.get_scaling,
            # rotations=self.nerfmodel.gaussians.get_rotation,
            cov3D_precomp=cov3D)
...
def _convert_vertex_colors_to_texture(
    ...
    # calculate approximate texture image(uv map) size
    ...
    with torch.no_grad():
        ...
        # broadcast adding, output shape (len(points_to_mesh), 2, 3)
        # TODO what the meaning of [[0,2,1],[0,3,2]]? and why multiply 4? 4 = square_size^2?
        ...
        # broadcast adding, output shape (vers_coords.shape[0], 4, 2)
        ...
        for i in range(n_square_per_axis):
            for j in range(n_square_per_axis):
                ...
                # store same color value in the square_size^2 tensor
                texture_img[...,
                ...
                # store same gaussian index in the square_size^2 tensor
                point_idx_per_pixel[...,
                start_idx_i:start_idx_i + square_size,
    ...
    # point idx means gaussians' index
...
def extract_texture_image_and_uv_from_gaussians(
    ...
    for cam_idx in range(len(rc.nerfmodel.training_cameras)):
        ...
        if not use_average:
            ...
            texture_img[(idx_to_update[..., 0], idx_to_update[..., 1])] = no_initialize_mask[..., None] * texture_img[
                (idx_to_update[..., 0], idx_to_update[..., 1])]

            texture_img[(idx_to_update[..., 0], idx_to_update[..., 1])] = texture_img[(
                idx_to_update[..., 0], idx_to_update[..., 1])] + rgb_img[update_mask]
            texture_counter[(idx_to_update[..., 0], idx_to_update[..., 1])] = texture_counter[(
